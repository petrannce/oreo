                        ...
                        <div class="body px-4 py-4">
                            <form action="{{ route('lab_tests.store') }}" method="POST">
                                ...
                                <div class="row">
                                    ...
                                    {{-- Lab Technician --}}
                                    @if(auth()->check() && in_array(auth()->user()->role, ['lab_technician', 'admin']))
                                        <div class="col-md-4 mb-3">
...
<div class="col-md-6 mb-3">
    ...
    @php
        // Build suggested test names from lab requirements (if any)
        $autoTestName = '';
        if (isset($selectedAppointment) && $selectedAppointment->labRequirements->count() > 0) {
            ...
            foreach ($selectedAppointment->labRequirements as $requirement) {
                // Split by commas, trim spaces, and merge all
                $names = array_merge($names, array_map('trim', explode(',', $requirement->name)));
            }
            $autoTestName = implode(', ', $names);
        }
    @endphp
    ...
    <input type="text"
           name="test_name"
           class="form-control"
           value="{{ old('test_name', $autoTestName) }}"
           @if($autoTestName) readonly @endif
           required>
                                ...
                                <div class="row">
                                    <div class="col-md-4 mb-3">
                                        ...
                                        <select name="status" class="form-control" required>
                                            <option value="requested" {{ old('status') == 'requested' ? 'selected' : '' }}>
                                                Requested</option>
