                                            ...
                                            <td>
                                                ...
                                                @if($stage === 'triage' && auth()->user()->hasAnyRole(['nurse', 'admin']))
                                                    @if($appointment->triage)
                                                        <a href="{{ route('triages.show', $appointment->triage->id) }}" class="btn btn-sm btn-success">
                                                            <i class="zmdi zmdi-check"></i> Triage Created
                                                        </a>
                                                    @else
                                                        <a href="{{ route('triages.create', ['appointment_id' => $appointment->id]) }}" class="btn btn-sm btn-info">
                                                ...
                                                @if($stage === 'doctor_consult' && auth()->user()->hasAnyRole(['doctor', 'admin']))
                                                    @if($appointment->medicalReport)
                                                        <a href="{{ route('medicals.show', $appointment->medicalReport->id) }}" class="btn btn-sm btn-success">
                                                            <i class="zmdi zmdi-check"></i> Consultation Done
                                                        </a>
                                                    @else
                                                        <a href="{{ route('medicals.create', ['appointment_id' => $appointment->id]) }}" class="btn btn-sm btn-primary">
                                                ...
                                                @if($stage === 'lab' && auth()->user()->hasAnyRole(['lab_technician', 'doctor', 'admin']))
                                                    @if($appointment->labTest)
                                                        <a href="{{ route('medicals.create', ['appointment_id' => $appointment->id]) }}" class="btn btn-sm btn-success">
                                                            <i class="zmdi zmdi-flask"></i> Lab Done
                                                        </a>
                                                    @else
                                                        <a href="{{ route('lab_tests.create', ['appointment_id' => $appointment->id]) }}" class="btn btn-sm btn-warning">
                                                ...
                                                @if($stage === 'pharmacy' && auth()->user()->hasAnyRole(['pharmacist', 'admin']))
                                                    ...
                                                    @else
                                                        <a href="{{ route('pharmacy_orders.create', ['appointment_id' => $appointment->id]) }}" class="btn btn-sm btn-secondary">
                                                ...
                                                @if($stage === 'billing' && auth()->user()->hasAnyRole(['pharmacist', 'admin']))
                                                    @if($appointment->billing)
                                                        <a href="{{ route('billings.show', $appointment->billing->id) }}" class="btn btn-sm btn-success">
                                                            <i class="zmdi zmdi-money"></i> Bill Generated
                                                        </a>
                                                        <small class="d-block text-muted">
                                                            Total: Ksh {{ number_format($appointment->billing->amount ?? 0, 2) }}
                                                        </small>
                                                    @else
                                                        <a href="{{ route('billings.create', ['appointment_id' => $appointment->id]) }}" class="btn btn-sm btn-dark">
